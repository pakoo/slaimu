'''Modified by observer@hoopchina

    * add find_eq method
    * modifies find_ge,find_gt,find_le,find_lt
...
class SortedCollection(object):
    ...
    def find_le(self, k, limit=None):
        'Return items with a key <= k.  Raise ValueError if not found.'
        i = bisect_right(self._keys, k)
        if limit is None:
            limit = i
        if i:
            return [self._items[x] for x in range(i-limit,i)]
        raise ValueError('No item found with key at or below: %r' % (k,))

    def find_lt(self, k, limit=None):
        'Return items with a key < k.  Raise ValueError if not found.'
        i = bisect_left(self._keys, k)
        if limit is None:
            limit = i
        if i:
            return reversed([self._items[x] for x in range(i-limit,i)])
        raise ValueError('No item found with key below: %r' % (k,))

    def find_ge(self, k, limit=None):
        'Return items with a key >= equal to k.  Raise ValueError if not found'
        i = bisect_left(self._keys, k)
        if limit is None:
            limit = len(self._items)-i
        if i != len(self):
            return [self._items[x] for x in range(i,i+limit)]
        raise ValueError('No item found with key at or above: %r' % (k,))

    def find_gt(self, k, limit=None):
        'Return items with a key > k.  Raise ValueError if not found'
        i = bisect_right(self._keys, k)
        if limit is None:
            limit = len(self._items)-i
    ...
    def find_eq(self, k):
        'Return all items with a key = k. Raise ValueError if not found'
        i = bisect_left(self._keys,k)
        if i != len(self):
            if self._keys[i] != k:
                raise ValueError('No item found with key equal: %r' % (k,))
            ret = [i]
            i += 1
            while i<len(self._keys) and self._keys[i] == k:
                ret.append( i )
                i += 1
            return [ self._items[i] for i in ret ]
        raise ValueError('No item found with key equal: %r' % (k,))
